...
# Load libraries used by the validation suite
from pyspark.sql.functions import *
from datetime import datetime
import time

# Set the validation-run configuration
...
# Declare the paths for each lakehouse layer
...
# Check that expected catalog tables are available
uc_tables = [
...
# Record baseline measures for later comparison
baseline_metrics = {}

print("\n BASELINE METRICS CAPTURE")
print("="*80)

# Capture Bronze table counts
...
# List the pipeline modules under test
pipeline_notebooks = [
...
# Confirm that data is present throughout the pipeline
...
# Check the star-schema key relationships
print("\n STAR SCHEMA INTEGRITY VALIDATION")
print("="*80)

# Load the fact table and its dimensions
...
# Store the state before the repeatability test
...
# Compare source and Bronze totals to assess repeatability
...
# Inspect Silver Type 2 structure without modifying data
...
# Confirm that Bronze retains source duplicates
...
# Confirm that Silver resolves duplicates through SCD processing
...
# `is_current` is a string field, so compare it with "True"
...
# Test uniqueness of Gold-layer surrogate keys
...
# Review audit records for quality failures
...
# Confirm Silver fields have the expected types
...
# Count nulls in required business fields
...
# Optional fields are reported without failing the check
...
# Test data values against business-rule limits
...
# Confirm audit logs record row-count information
...
# Compare audit totals with the stored datasets
...
# Gather basic code-quality indicators
print("\n CODE QUALITY REVIEW")
print("="*80)
...
# Check for the expected Delta Lake practices
...
# Verify that the required project documentation exists
...
# Outline the evidence-capture process
...
# Run the final compliance checklist
print("\n" + "="*80)
...
# Consolidate the validation outcomes
print("\n" + "="*80)
...
# Produce the final project status summary
...
# List optional next improvements
print("\n" + "="*80)
...
# Build a report that can be exported
print("\n" + "="*80)
