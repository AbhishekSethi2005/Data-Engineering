...
# Prepare audit folders within the Raw area
...
# Move the incoming change files
dbutils.fs.cp(
...
# Place the Landing audit inputs
dbutils.fs.cp(
...
# List the configured Raw-layer directories
for folder in [
    "historical/customer",
    "historical/product",
    "historical/sales",
...
# Load the source using that declared schema
customer_raw_df = (
    spark.read
    .option("header", True)
...
# Write the Landing representation as Parquet
...
# Assemble the full Landing ingestion routine
from pyspark.sql.types import StructType, StructField, StringType
...
# Examine a representative audit file
...
# Reconcile Landing counts dynamically
from pyspark.sql.functions import col

# Load the audit reference
audit_path = "/Volumes/workspace/default/celebal_data/raw/audit_landing/customer_historical_audit.csv"
...
# Retrieve the expected row total
expected_count = int(
    audit_df.filter(
...
# Apply reconciliation to all six Landing datasets
from pyspark.sql.functions import col
...
# Build the append-only Bronze Delta layer
...
# Load the three historical entities into Bronze
landing_base = "/Volumes/workspace/default/celebal_data/landing"
...
# Ready the Bronze tables for incremental appends
landing_base = "/Volumes/workspace/default/celebal_data/landing"
bronze_base = "/Volumes/workspace/default/celebal_data/bronze"
...
# Identify the fields that differ between batches
for table in ["customer", "product", "sales"]:
...
# Inspect the incoming customer and product records
for table in ["customer", "product"]:
    print(f"\n===== {table.upper()} INCREMENTAL =====")
...
# Add ingestion timestamps to Bronze data
...
# Apply the ingestion timestamp to every Bronze table
from pyspark.sql.functions import current_timestamp
...
# Execute the incremental Bronze load
from pyspark.sql.functions import current_timestamp
...
# Read the incremental sales batch
incremental_sales_df = spark.read.parquet(incremental_sales_path)

# Stamp each arriving record with its load time
incremental_sales_df = incremental_sales_df.withColumn(
    "ingested_at",
    current_timestamp()
)

# Append records without changing prior Bronze rows
(
...
# Confirm that Bronze retains append-only behaviour
bronze_sales_df = (
    spark.read
    .format("delta")
    .load(bronze_sales_path)
...
# Prepare the customer incremental load for Bronze
# Add the required missing fields to Bronze
...
# Add nullable SCD fields to the historical records
bronze_customer_df = (
    bronze_customer_df
...
# Insert new rows only; do not update or delete history
(
...
# Align the product incremental schema
from pyspark.sql.functions import lit
...
# Append the product batch to Bronze
...
# Define a reusable audit-check routine
from pyspark.sql.functions import col
...
# Perform the final Bronze checks
from pyspark.sql.functions import col
