...
# Compare the incoming row totals
for name, df in {
...
# Apply the Silver cleansing rules
# Records with absent primary keys
...
# Remove repeated business records
...
# Cast fields to their intended types
# Customer
from pyspark.sql.functions import col
...
# Treat null values in the customer data
...
# Set the sales field data types
sales_clean.printSchema()

# COMMAND ----------

# Apply null-handling rules across all entities
#Customer
customer_clean = (
    customer_clean
    .fillna("Unknown")
...
# Create the initial Silver Delta tables
# Set up the Silver storage path
# Save the customer Silver table
...
# Validate the persisted Silver tables
...
# Implement Delta-based change processing
# Register the Silver tables for SQL access
...
# Verify the retained customer history
from pyspark.sql.functions import col
...
# Apply the customer Type 2 history process
# Check the records marked as current
from pyspark.sql.functions import col
...
customer_clean.groupBy("customer_id") \
    ...
    .orderBy("customer_id") \
    .show(20, False)
# Review the historical customer rows
...
# Confirm the required SCD attributes
customer_clean.select(
    "customer_id",
    "version",
    "effective_start_date",
...
# Apply the product Type 1 update policy
# Check the product business key
from pyspark.sql.functions import countDistinct
...
# Confirm overwrite behaviour for product updates
product_clean.columns

# COMMAND ----------

# Review the final product result
...
# Preserve the sales ledger while resolving duplicates with a window
...
# Define the window used for duplicate selection
sales_window = (
    Window
...
# Retain one valid version of each transaction
sales_final = (
    sales_clean
    .withColumn(
        "row_num",
...
# Confirm no duplicate transactions remain
duplicates = (
    sales_final
...
# Create warehouse surrogate identifiers
# Import the required helper
from pyspark.sql.functions import monotonically_increasing_id

# COMMAND ----------

# Assign customer surrogate keys
customer_final = (
    customer_clean
    .withColumn(
        "customer_sk",
...
# Check that each surrogate key is unique
from pyspark.sql.functions import countDistinct
...
# Persist the completed Silver tables
# Customer
...
# Run the final Silver audit checks
# Reconcile Bronze and Silver row totals
...
# Perform entity-level duplicate checks
# Customer
